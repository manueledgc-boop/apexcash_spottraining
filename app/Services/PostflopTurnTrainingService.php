...
class PostflopTurnTrainingService
{
    public function __construct(protected PostflopTurnSpotRepository $spots)
    {
    }
    ...
    public function nextSpot(?string $module = null, string $mode = 'normal', ?string $spotId = null, ?string $concept = null): array
    {
        ...
        session(['postflop_turn_training.current_spot' => $spot]);
    ...
    {
        $spot = session('postflop_turn_training.current_spot');
    ...
    public function evaluateAnswer(string $answer): array
    {
        ...
        if (! is_array($spot)) {
            return [
                'success' => false,
                'code' => 'NO_ACTIVE_POSTFLOP_TURN_SPOT',
                'message' => 'No hay spot de turn activo. Genera un nuevo spot.',
            ];
        }
        ...
        session()->push('postflop_turn_training.results', $sessionResult);
    ...
    {
        $results = session('postflop_turn_training.results', []);
        $total = count($results);
    ...
    {
        $trainingSessionId = session('postflop_turn_training.training_session_id');
        ...
        session()->forget('postflop_turn_training');
    }
    ...
    protected function persistResult(array $spot, string $answer, string $correctAction, string $grade, bool $isCorrect, ?int $frequency, int $evScore, int $xpEarned, string $explanation): void
    {
        ...
        $this->updateStat($userId, 'postflop_turn', 'Postflop Turn', $grade, $isCorrect, $xpEarned);
    ...
    {
        $userId = Auth::id();
        $sessionId = session('postflop_turn_training.training_session_id');
        ...
        session(['postflop_turn_training.training_session_id' => $trainingSession->id]);
    ...
    protected function publicSpot(array $spot): array
    {
        return [
            ...
            'difficulty' => $spot['difficulty'] ?? 'Turn V1',
