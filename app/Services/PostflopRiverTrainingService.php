...
class PostflopRiverTrainingService
{
    public function __construct(protected PostflopRiverSpotRepository $spots)
    {
    }
    ...
    public function nextSpot(?string $module = null, string $mode = 'normal', ?string $spotId = null, ?string $concept = null): array
    {
        ...
        session(['postflop_river_training.current_spot' => $spot]);
    ...
    {
        $spot = session('postflop_river_training.current_spot');
    ...
    public function evaluateAnswer(string $answer): array
    {
        ...
        if (! is_array($spot)) {
            return [
                ...
                'message' => 'No hay spot de river activo. Genera un nuevo spot.',
            ];
        }
        ...
        session()->push('postflop_river_training.results', $sessionResult);
    ...
    {
        $results = session('postflop_river_training.results', []);
        $total = count($results);
    ...
    {
        $trainingSessionId = session('postflop_river_training.training_session_id');
        ...
        session()->forget('postflop_river_training');
    }
    ...
    protected function persistResult(array $spot, string $answer, string $correctAction, string $grade, bool $isCorrect, ?int $frequency, int $evScore, int $xpEarned, string $explanation): void
    {
        ...
        $this->updateStat($userId, 'postflop_river', 'Postflop River', $grade, $isCorrect, $xpEarned);
    ...
    {
        $userId = Auth::id();
        $sessionId = session('postflop_river_training.training_session_id');
        ...
        session(['postflop_river_training.training_session_id' => $trainingSession->id]);
    ...
    protected function publicSpot(array $spot): array
    {
        return [
            ...
            'difficulty' => $spot['difficulty'] ?? 'River V1',
