...
class MasteryTrainingService
{
    ...
    {
        $spot = session('mastery_training.current_spot');
    ...
    public function evaluateAnswer(string $answer): array
    {
        ...
        if (! is_array($spot)) {
            return [
                'success' => false,
                'code' => 'NO_ACTIVE_MASTERY_SPOT',
                'message' => 'No hay spot de mastery activo. Genera un nuevo spot.',
            ];
        }
        ...
        session()->push('mastery_training.results', $sessionResult);
    ...
    {
        $results = session('mastery_training.results', []);
        $total = count($results);
    ...
    {
        $trainingSessionId = session('mastery_training.training_session_id');
        ...
        session()->forget('mastery_training');
    }
    ...
    protected function persistResult(array $spot, string $answer, string $correctAction, string $grade, bool $isCorrect, ?int $frequency, int $evScore, int $xpEarned, string $explanation): void
    {
        ...
        $this->updateStat($userId, 'mastery_global', 'Mastery', $grade, $isCorrect, $xpEarned);
    ...
    {
        $userId = Auth::id();
        $sessionId = session('mastery_training.training_session_id');
        ...
        session(['mastery_training.training_session_id' => $trainingSession->id]);
    ...
    protected function publicSpot(array $spot): array
    {
        return [
            ...
            'difficulty' => $spot['difficulty'] ?? 'Mastery',
    ...
    protected function moduleLabel(string $module): string
    {
        return match ($module) {
            ...
            'blind_vs_blind_advanced' => 'Blind vs Blind Advanced',
            'multiway' => 'Multiway',
