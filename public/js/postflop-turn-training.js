(() => {
    ...
    function renderSpot() {
        ...
        els.spotTitle.textContent = spot.title ?? 'Spot turn';
        els.spotMeta.textContent = `${spot.street?.toUpperCase() ?? 'TURN'} · ${spot.hero_position} vs ${spot.villain_position} · ${spot.difficulty ?? 'Turn V1'} · confianza ${spot.confidence ?? 0}%`;
    ...
    function renderLeaks(leaks) {
        if (!Array.isArray(leaks) || leaks.length === 0) {
            els.leaksList.innerHTML = '<p class="spot-meta">Aún no hay suficientes manos de turn para detectar leaks.</p>';
            return;
        }
