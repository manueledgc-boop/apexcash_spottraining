(() => {
    const state = {
        spot: window.ApexPostflopTurnTraining?.initialSpot ?? null,
        summary: window.ApexPostflopTurnTraining?.initialSummary ?? {},
        leaks: window.ApexPostflopTurnTraining?.initialLeaks ?? [],
        currentModule: window.ApexPostflopTurnTraining?.initialModule ?? '',
        locked: false,
    };

    const config = window.ApexPostflopTurnTraining ?? {};
    ...
    function showFreeLimit(message) {
        ...
        if (els.feedbackBox) {
            ...
            els.feedbackBox.innerHTML = `
                ...
                <p>${message || 'Has completado el límite gratuito de Turn. Actualiza a Premium para continuar.'}</p>
    ...
    function renderSpot() {
        ...
        els.spotTitle.textContent = spot.title ?? t('default_title', 'Turn spot');
        els.spotMeta.textContent = `${spot.street?.toUpperCase() ?? 'TURN'} · ${spot.hero_position} vs ${spot.villain_position} · ${spot.difficulty ?? t('default_difficulty', 'Turn V1')} · ${t('confidence', 'confidence')} ${spot.confidence ?? 0}%`;
    ...
    function renderLeaks(leaks) {
        if (!Array.isArray(leaks) || leaks.length === 0) {
            els.leaksList.innerHTML = `<p class="spot-meta">${t('no_leaks', 'Not enough turn hands yet to detect leaks.')}</p>`;
            return;
        }
    ...
    function hideResult() {
        if (els.textureBox) {
            els.textureBox.hidden = true;
            els.textureBox.style.display = '';
        }

        els.decisionResultBox.hidden = true;
        els.decisionResultBox.style.display = '';
    ...
    async function nextSpot() {
        if (state.locked && state.spot === null) return;
        ...
        try {
            ...
            if (!data.success) {
                throw new Error(data.message ?? t('next_error', 'Could not load the next spot.'));
            }
